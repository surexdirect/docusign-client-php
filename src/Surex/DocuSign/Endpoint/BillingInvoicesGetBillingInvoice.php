...
class BillingInvoicesGetBillingInvoice extends \Jane\OpenApiRuntime\Client\BaseEndpoint implements \Jane\OpenApiRuntime\Client\Psr7HttplugEndpoint
{
    ...
    */
    function __construct(string $accountId, string $invoiceId)
    {
    ...
    {
        return str_replace(array('{accountId}', '{invoiceId}'), array($this->accountId, $this->invoiceId), '/v2/accounts/{accountId}/billing_invoices/{invoiceId}');
    }
     ...
     *
     * @return null|\Surex\DocuSign\Model\Invoices
     */
    protected function transformResponseBody(string $body, int $status, \Symfony\Component\Serializer\SerializerInterface $serializer)
    {
        if (200 === $status) {
            return $serializer->deserialize($body, 'Surex\\DocuSign\\Model\\Invoices', 'json');
        }
        if (400 === $status) {
            throw new \Surex\DocuSign\Exception\BillingInvoicesGetBillingInvoiceBadRequestException($serializer->deserialize($body, 'Surex\\DocuSign\\Model\\ErrorDetails', 'json'));
        }
    }
}